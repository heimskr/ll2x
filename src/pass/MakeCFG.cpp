...
namespace LL2X::Passes {
	...
	void walkCFG(Function &function, size_t walks, unsigned int seed, size_t inner_limit) {
		Timer timer("WalkCFG");
		std::default_random_engine rng(seed == 0? std::chrono::system_clock::now().time_since_epoch().count() : seed);
		...
		for (size_t walk = 0; walk < walks; ++walk) {
			...
			while (node != end && ++count <= inner_limit) {
				...
				if (out_count == 1)
					...
					node = *std::next(node->out().begin(),
						std::uniform_int_distribution<int64_t>(0, static_cast<int64_t>(out_count - 1))(rng));
			}
		}
